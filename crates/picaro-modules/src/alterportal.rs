...
const REFERER: &str = "https://alterportal.net/";

pub fn module_information() -> ModuleInformation {
    ModuleInformation {
        service_name: "AlterPortal".to_string(),
        ...
        netlocation_constant: NetlocConstants::Single("alterportal.net".to_string()),
        url_constants: {
            let mut m = indexmap::IndexMap::new();
            m.insert("album".to_string(), DownloadType::album);
            m.insert("singles".to_string(), DownloadType::track);
            m.insert("2026_albums".to_string(), DownloadType::album);
            m.insert("2025_albums".to_string(), DownloadType::album);
            m.insert("alternative".to_string(), DownloadType::album);
            m.insert("core".to_string(), DownloadType::album);
            m.insert("metal".to_string(), DownloadType::album);
            m.insert("rock".to_string(), DownloadType::album);
            m.insert("punk".to_string(), DownloadType::album);
            m.insert("industrial".to_string(), DownloadType::album);
            m.insert("experimental".to_string(), DownloadType::album);
            m.insert("exussr".to_string(), DownloadType::album);
            m.insert("lossless".to_string(), DownloadType::album);
            m.insert("newsongs".to_string(), DownloadType::track);
            m.insert("va".to_string(), DownloadType::album);
            m.insert("discography".to_string(), DownloadType::album);
            m
        },
        test_url: Some(
            "https://alterportal.net/2026_albums/12345-test-album-2026.html".to_string(),
        ),
...
pub fn constructor() -> Arc<dyn ModuleConstructor> {
    Arc::new(AlterPortalConstructor)
}

#[derive(Debug)]
struct AlterPortalConstructor;

impl ModuleConstructor for AlterPortalConstructor {
    fn construct(&self, controller: ModuleController) -> Result<ModuleInterfacePtr> {
        Ok(Arc::new(AlterPortalModule {
            controller,
...
async fn fetch_album_page(client: &reqwest::Client, url: &str) -> Result<String> {
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| Error::Other(format!("alterportal album fetch: {e}")))?;
    if !resp.status().is_success() {
        return Err(Error::Other(format!(
            "alterportal album HTTP {}",
            resp.status()
        )));
    }
    resp.text()
        .await
        .map_err(|e| Error::Other(format!("alterportal album read: {e}")))
}

fn parse_album_meta(html: &str) -> (String, String, String, Option<String>, Option<i32>) {
    let title_re = Regex::new(r"<title>(.*?)\s*Â»\s*Alterportal.*</title>").unwrap();
    let title = title_re
        .captures(html)
...
fn parse_search_results(html: &str) -> Vec<SearchResult> {
    let title_re =
        Regex::new(r#"<a href="(https://alterportal\.net/[a-z_]+/\d+-[^"]+)"[^>]*>([^<]+)</a>"#)
            .unwrap();
    let year_re = Regex::new(r"\((\d{4})").unwrap();
...
#[async_trait]
impl picaro_utils::module::ModuleInterface for AlterPortalModule {
    fn name(&self) -> &str {
        "AlterPortal"
    }
    ...
    async fn get_track_download(
        ...
        let direct_url = if track_id.starts_with("https://") {
            ...
            return Err(Error::Other(format!(
                "alterportal: expected direct download URL, got {track_id}"
            )));
        };
    ...
    async fn get_album_info(
        ...
        let url = if album_id.starts_with("https://") {
            ...
            format!("https://alterportal.net/{album_id}")
        };
        ...
        let flac_url = parse_download_link(&html, "FLAC").ok_or_else(|| {
            Error::Other("alterportal: FLAC download link not found on album page".to_string())
        })?;
        Ok(AlbumInfo {
            name: album,
            artist: artist.clone(),
    ...
    async fn get_track_cover(
        ...
        Err(Error::Other(format!(
            "alterportal: no cover for track {track_id}"
        )))
    }
    ...
    async fn search(
        ...
        let mut all = Vec::new();
        let max_pages = ((limit as usize + 9) / 10).min(5);
        for page in 1..=max_pages {
            let encoded = utf8_percent_encode(query, NON_ALPHANUMERIC).to_string();
            let url = format!(
 "https://alterportal.net/index.php?do=search&subaction=search&story={}&search_start={}",
 encoded, page
 );
            let resp = self
                .client
                ...
                .send()
                .await
                .map_err(|e| Error::Other(format!("alterportal search: {e}")))?;
            if !resp.status().is_success() {
                break;
            }
            let html = resp
                .text()
                .await
                .map_err(|e| Error::Other(format!("alterportal search read: {e}")))?;
