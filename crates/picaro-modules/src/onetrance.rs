//! 1Trance (1trance.org) site module.
//!
//! Drupal 8 download blog for trance / progressive.
//! Search:   GET https://1trance.org/?t=<query>  (the "t" exposed filter view)
...
const REFERER: &str = "https://1trance.org/";
const BASE: &str = "https://1trance.org";
const SERVICE: &str = "1Trance";
...
pub fn module_information() -> ModuleInformation {
    ...
    ModuleInformation {
        ...
        url_constants,
        test_url: Some("https://1trance.org/?t=techno".to_string()),
...
pub fn constructor() -> Arc<dyn ModuleConstructor> {
    Arc::new(OneTranceConstructor)
}

#[derive(Debug)]
struct OneTranceConstructor;

impl ModuleConstructor for OneTranceConstructor {
    fn construct(&self, controller: ModuleController) -> Result<ModuleInterfacePtr> {
        Ok(Arc::new(OneTranceModule {
            controller,
...
async fn fetch_page(client: &reqwest::Client, url: &str) -> Result<String> {
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| Error::Other(format!("1trance fetch: {e}")))?;
    if !resp.status().is_success() {
        return Err(Error::Other(format!("1trance HTTP {}", resp.status())));
    }
    resp.text()
        .await
        .map_err(|e| Error::Other(format!("1trance read: {e}")))
}
...
#[async_trait]
impl picaro_utils::module::ModuleInterface for OneTranceModule {
    fn name(&self) -> &str {
        SERVICE
    }
    ...
    async fn get_track_download(
        ...
        if !track_id.starts_with("http") {
            return Err(Error::Other(format!(
                "1trance: expected file-host URL, got {track_id}"
            )));
        }
    ...
    async fn get_album_info(
        ...
        if links.is_empty() {
            return Err(Error::Other(format!(
                "1trance: no file-host links found at {url}"
            )));
        }
    ...
    async fn get_track_cover(
        ...
        Err(Error::Other(format!(
            "1trance: no cover for track {track_id}"
        )))
    }
    ...
    async fn search(
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| Error::Other(format!("1trance search: {e}")))?;
        if !resp.status().is_success() {
            return Ok(Vec::new());
        }
        let html = resp
            .text()
            .await
            .map_err(|e| Error::Other(format!("1trance search read: {e}")))?;
